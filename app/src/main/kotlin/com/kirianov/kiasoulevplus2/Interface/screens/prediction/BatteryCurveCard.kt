...
private fun DistanceChart(points: List<DistancePoint>, lineColor: Color, gridColor: Color) {
    ...
    Text(
        text = if (points.isEmpty()) {
            ...
            "Нуль угорі, тож довгі відсотки провисають. Провал справа — не манера " +
                "їзди, а сама шкала: на замірі 97 % → 0.4 % останні вісім відсотків " +
                "дали 5.1 км кожен проти 2.4 км у решти, і з цієї різниці лише " +
                "десята частина пояснюється економнішою їздою. Решта — енергія, " +
                "яку BMS ховає внизу шкали, рахуючи відсотки за рідним пакетом."
        },
        style = MaterialTheme.typography.bodySmall,
    )
}
