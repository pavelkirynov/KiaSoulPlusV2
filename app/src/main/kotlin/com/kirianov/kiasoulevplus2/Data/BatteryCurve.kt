...
data class BatteryCurve(
    ...
    /**
     * ПРОГНОЗ НА ЦІЙ КРИВІЙ БІЛЬШЕ НЕ БУДУЄТЬСЯ — НІ НА A, НІ НА B.
     *
     * Спершу прогноз брав криву A, потім лише B, і обидва рази результат був
     * гіршим за модель. Замір настінною станцією показав, наскільки: зарядка від
     * 0.2 % до 98.3 % узяла з розетки 50.81 кВт·год — у комірки прийшло 43-45, —
     * а криві казали 26.5 (A) і 29.6 (B) на всю шкалу. Модель у той самий момент
     * казала 40.0.
     *
     * Чому криві низять, видно поштучно в журнальних рядках «curve+»: майже всі
     * інтервали, які до них потрапляють, починаються з 98 %, тобто одразу після
     * відключення від зарядки. Там BMS переоцінює SOC униз, і відсотки сиплються
     * швидше, ніж іде енергія: 0.33 кВт·год на відсоток проти 0.52 в інтервалі,
     * знятому здалеку від зарядки.
     *
     * Криві лишаються на графіку — вони саме цю розбіжність і показують, — але
     * прогноз бере ємність із моделі. Див. MlBlock.
     */
