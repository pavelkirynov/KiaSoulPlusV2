...
/**
 * Прохання записати шину протягом [seconds]: слухати довгим вікном і вести журнал
 * того, що на ній зʼявляється й міняється.
 *
 ...
 * один зріз.
 *
 * [filterId] — необовʼязковий фільтр на один CAN ID. Порожній — слухати всю шину,
 * але тоді адаптер вимушено ділить час на короткі вікна й між ними лишаються
 * провали: швидке перемикання (замок 433 туди-сюди) може випасти в діру. Заданий
 * ID знімає провали: адаптер віддає рівний потік одного кадру довгим вікном, і в
 * журналі видно кожне перемикання без пропусків. Ним ловлять уже виявленого
 * підозрюваного, коли треба підтвердити, який біт смикає кнопка.
 */
data class RecordRequest(val seconds: Int, val sequence: Long, val filterId: String = "")
...
data class BusRecording(
    ...
    /** На який CAN ID стояв фільтр під час запису. Порожній — слухали всю шину. */
    val filterId: String = "",
) {
