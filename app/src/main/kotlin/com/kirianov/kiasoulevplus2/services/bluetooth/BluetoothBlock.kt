...
class BluetoothBlock(private val bluetoothManager: ElmBluetoothManager) {
    ...
    internal companion object {
         ...
         *
         * ПРАВИЛО ЧЕРГИ: 4F0 стоїть ЧЕРЕЗ ОДИН. Пробіг і швидкість — основа кожного
         * відрізка витрати, і їхня частота не має права падати від того, що ми
         * додали в чергу ще один кадр. Тому кожен новий ID приходить зі своїм 4F0
         * у парі, і додавання кадру розтягує період решти, а не пробігу.
         *
         * Вікно займає близько п'яти секунд. Тридцять шість записів — цикл близько
         * трьох хвилин, у якому:
         *
         *   4F0 (пробіг, швидкість) — 18 разів, ~10 с. Основа кожного відрізка.
         *   598 (точний SOC)        — 4 рази,  ~45 с. Крива ємності вивчається з
         *                             пар «крок SOC / енергія», тож рідкий SOC
         *                             прямо стримує навчання.
         *   594 (SOC панелі)        — 2 рази,  ~90 с.
         *   200 (запас + клімат)    — 2 рази,  ~90 с.
         *   581 (заряджання)        — 2 рази,  ~90 с. Треба, щоб вчасно розділити
         *                             рух і заряд.
         *   653 (температура)       — 2 рази,  ~90 с.
         *   4B0 (колеса)            — 2 рази,  ~90 с. Нове, з SoulEVSpy.
         *   4F2 (швидкість, ключ)   — 2 рази,  ~90 с. Нове; тут біт запалювання.
         *   433 (ручник)            — 1 раз,   ~180 с. Нове.
         *   050 (світло, щітки)     — 1 раз,   ~180 с. Нове.
         *   567 (годинник)          — 1 раз,   ~180 с. Нове.
         *
         * ЧОМУ НОВІ КАДРИ СТОЯТЬ РІДКО. Вони поки не живлять нічого — їх додано,
         * щоб З'ЯСУВАТИ, чи взагалі приходять і чи так у них розкладені байти. На
         * це питання відповідає одне влучання за цикл, а частішим опитуванням ми
         * платили б швидкістю SOC, за який тримається крива ємності. Як живий
         * покажчик поворотника кадр 050 у такій черзі не годиться, і це навмисно.
         *
         * Годинник (567) сюда вернувся саме як така перевірка, а не як джерело
         * часу: за джерело часу магнітолу вже відкинуто, і діагностику дрейфу
         * відкотили. Секунди, що рахують одна за одною, — найдешевша перевірка
         * того, що байти кадру взято правильно.
         */
        val MONITOR_ROTATION = listOf(
            "4F0", "598", "4F0", "4B0", "4F0", "594", "4F0", "4F2", "4F0", "200",
            "4F0", "433", "4F0", "598", "4F0", "581", "4F0", "653",
            "4F0", "598", "4F0", "4B0", "4F0", "594", "4F0", "4F2", "4F0", "200",
            "4F0", "050", "4F0", "598", "4F0", "581", "4F0", "567",
        )
    }
}
