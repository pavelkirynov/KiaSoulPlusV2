        ...
        val values = buildList {
            ...
            add(
                PortedValue(
                    ...
                    // справно; невідомо, що він означає, і два заміри вже
                    // виключили обидві очевидні відповіді: на стоянці він був
                    // нулем (для ручника мало б бути навпаки), а на 59.5 км/год —
                    // одиницею (педаль так довго не тримають).
                    state = verdict(systems.brake.known, false),
                    note = "SoulEVSpy зве це ручником, але в журналі біт стояв на нулі на " +
                        "стоянці й на одиниці на 59.5 км/год — ні ручник, ні педаль так не " +
                        "поводяться. Що це, поки невідомо",
                ),
            )
