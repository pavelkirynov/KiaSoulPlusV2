...
class CapacityModel(
     ...
     *
     * Чому не поліном, а корзини. Комірки в пакеті — не рідні, а BMS читає їх
     * таблицею напруг від рідних. Наскільки крива через це «поїде» по шкалі й де
     * саме — наперед невідомо: залежить від того, які комірки поставлені. Форму
     * доводиться не припускати, а міряти, і корзини для цього придатні, бо не
     * нав'язують жодної форми взагалі.
     *
     * Поліном же нав'язує, і найгірше поводиться **поза** тим діапазоном, де були
     * дані: на перевірці кривою з вираженим горбом куб дав −79 % на 95 % шкали
     ...
     * їздили**, а не по всій шкалі. Якщо крива виявиться нерівною — а на чужих для
     * BMS комірках вона такою й буде, — то в того, хто тримає заряд між сорока й
     * сімдесятьма, це число зміститься в бік щільнішої середини. Крива такої вади
     * не має, бо пам'ятає, де саме була кожна сесія, — і саме тому на екрані стоять
     * обидва числа.
     *
    ...
    /**
     * Чи міряли вже краї шкали, чи дно й стеля — це поки апріорна пряма.
     *
     * Різниця для водія принципова: «4 % SOC» без жодної пари «панельний / точний»
     * — це не про його авто, а те, з чого модель починає. Показувати таке числом
     * без застереження означало б видавати припущення за вимір.
     */
    val scaleMeasured: Boolean get() = buffer.effectiveSamples > 0.0

    /** Чи була хоч одна сесія, з якої рахувалася ємність. */
    val capacityMeasured: Boolean get() = energy.effectiveSamples > 0.0
 ...
 * перетворював виміряні 45 кВт·год на 20 — тобто більш ніж удвічі занижував пакет.
 *
