...
internal object MlCodec {
    ...
    /**
     * Покоління моделі разом із ємністю пакета, з якою вона зібрана.
     *
     * Ємність тут не для порядку. Вона входить у модель апріорі — тим числом, з
     * якого стартують усі корзини шкали, поки їх не перекриють заміри. Зібрана на
     * рідних 27 кВт·год модель під написом «51» обіцяла б удвічі більше рівно до
     * першої глибокої зарядки, і помітити це було б нізвідки. Тому зміна ємності
     * робить збережений знімок чужим, і модель чесно збирається з журналу наново.
     *
     * Десята частка кіловат-години — достатня роздільність: дрібніше ніхто не
     * задає, а зайва точність робила б кожен запуск новим поколінням.
     */
    fun featureSetFor(nominalCapacityKwh: Double): String =
        "$FEATURE_SET@${kotlin.math.round(nominalCapacityKwh * 10.0) / 10.0}"
