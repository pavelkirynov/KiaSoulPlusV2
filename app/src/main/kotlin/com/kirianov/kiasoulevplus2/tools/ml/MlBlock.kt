...
class MlBlock(
    ...
    private fun publish() {
        ...
        // ЗАЛИШОК ЕНЕРГІЇ БЕРЕ МОДЕЛЬ, А НЕ КРИВА ЄМНОСТІ.
        //
        // Обидві знають розподіл ємності по шкалі — без цього прогноз на цій
        // машині взагалі не має сенсу, бо шкала в неї різко нерівна. Але
        // рахують вони його різними приладами, і після заміру настінною
        // станцією видно, який із них правий.
        //
        // Зарядка від 0.2 % до 98.3 % узяла з розетки 50.81 кВт·год — тобто в
        // комірки прийшло 43-45. Модель у той самий момент казала 40.0 кВт·год
        // робочої ємності, крива за струмом — 29.6. Помилка моделі 10 %, помилка
        // кривої 33 %; на екрані це виглядало як «корисної енергії 25.2» і 176 км
        // там, де вчора та сама машина проїхала 271 км від 97 % до 0.4 %.
        //
        // Чому крива низить, теперь відомо поштучно — рядки «curve+» у журналі.
        // Інтервали, які до неї потрапляють, майже всі починаються з 98 %, тобто
        // одразу після відключення від зарядки: там BMS переоцінює SOC униз, і
        // відсотки сиплються швидше, ніж іде енергія. Такі інтервали дають 0.33
        // кВт·год на відсоток проти 0.52 в інтервалі, знятому здалеку від
        // зарядки. Доки це не полагоджено, крива лишається на графіку — вона
        // саме цю розбіжність і показує, — але прогноз на ній не будується.
