...
/**
 * Нерівна крива шкали: скільки кіловат-годин стоїть за відсотком BMS у різних
 * місцях діапазону.
 *
 * Звідки вона береться. Комірки в пакеті не рідні, а відсоток BMS рахує за
 * таблицею напруг рідних. Там, де нова крива напруги йде пологіше за ту, на яку
 * BMS налаштована, вона «не помічає» витраченого — на малий ΔSOC припадає багато
 * енергії; там, де крутіше, навпаки. Наскільки сильно й де саме — залежить від
 * того, які комірки поставлені, і наперед цього не знає ніхто.
 *
 * Тому форма тут узята навмисно **сильніша за очікувану**: виражений горб
 * посередині. Це стрес-перевірка. Модель не мусить вгадувати форму — вона мусить
 * відновити ту, яка є, якою б вона не вийшла; на майже прямій кривій задача тільки
 * легша.
 *
 * Еталон навмисно **не поліном і не сума корзин** — гаусів горб із числовим
 * інтегруванням. Інакше тест перевіряв би, чи вміє модель вгадати власний базис,
 * а не чи ловить вона справжню форму.
 */
class ScaleCurveTest {
     ...
     */
    @Test
    fun `reconstructs a strongly humped scale curve`() {
     ...
     * шкалі. Тут середина енергетично щільніша за краї, тож у того, хто тримає
