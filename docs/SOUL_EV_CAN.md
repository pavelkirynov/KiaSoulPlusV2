...
|---|---|---|---|
| `4B0` | швидкості **чотирьох коліс** окремо | по 16 біт LE ÷ 30 → км/год; пари (1,0)(3,2)(5,4)(7,6) | кадр іде, числа правдоподібні; дільник під питанням (див. нижче) |
| `433` | гальмо: біт 3 байта 2. У SoulEVSpy це «ручник»; у нас він стояв на 0 на стоянці й піднявся рівно перед рухом — схоже радше на **педаль** | байт 2, біт 3 | біт читається; що означає — ні |
| `050` | світло (байт 1, маска 0x03), склоочисники (байт 2, маска 0x07 + байт 1, маска 0xF0), покажчики (байт 2, маска 0x30) | біти | кадр іде: `light=Automatic turn=Off wipe=Off` при вимкненому світлі в режимі «авто» |
...
| `4F2` | швидкість (байт 1 ÷ 2); біти 6-7 байта 2 — **не запалювання**, див. нижче | див. нижче: їхню формулу швидкості виправлено | швидкість — **так**, збіг із `4F0` до десятої |
...
| біти 6-7 байта 2 в `4F2` | 0 на 91.5 км/год, 1 на нерухомій машині | **не запалювання**. При повороті ключа вони таки змінюються з 00 на C0 — на це ми й купилися, — але ключ так не поводиться. Найпевніше дволітерний лічильник кадру, тому тепер зберігається значення 0-3, а не «так/ні» |
| колеса, `4B0` | 102.5 при `v4F0=92`; 21.1 при 18.5 | **числа правдоподібні, дільник під питанням**: двічі підряд колеса вийшли на 11-14 % вище. Або це різниця в часі зняття двох вікон монітора (вони йдуть за 5 с одне від одного), або дільник має бути 33.6, а не 30. Вирішить рівна швидкість на трасі: журнал тепер пише `x=` — відношення колес до `4F0` |
