...
| `433` | біт 3 байта 2. У SoulEVSpy це «ручник» — **точно не він** | байт 2, біт 3 | біт читається; **що означає — невідомо**: 0 на стоянці, 1 на 59.5 км/год. Ні ручник, ні педаль так не поводяться |
...
| колеса, `4B0` | 23 пари за день: відношення до `4F0` від 0.56 до 1.61, серединне ~1.03 | **працює, дільник 30 правильний**. Перші два заміри дали +11 і +14 % і виглядали як хибний дільник; на купі замірів розкид виявився ДВОБІЧНИМ, а хибний дільник дав би постійний перекіс. Причина розкиду — вікна монітора знімаються за 5-10 с одне від одного, і на розгоні швидкість за цей час змінюється |
...
І перший же журнал це виправдав. Усі відповіді до одної були такі:

```
tpms? «7F 22 78   >» bar=-
```

`7F 22 78` — це не «немає такого запиту» і не «не той блок». Це **«прийняв, готую
відповідь»** (0x78) на службу 0x22, тобто блок живий, запит зрозумів і просить
дати йому мить. Ту саму відмову ми вже бачили в опитуванні помилок, і там її
обробляє повтор; сюда його забули додати — тиск питався напряму, без повтору.

Тепер запит тиску йде через ту саму `ask` з ТРЬОМА повторами, а не одним: цей
блок відповів «готую» на кожен перший запит за весь журнал.
