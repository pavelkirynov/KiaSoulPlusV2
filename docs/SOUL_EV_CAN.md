...
| `567` | годинник авто | `b1`=год, `b2`=хв, `b3`=сек. Читається, щоб побачити зсув часу під РЕБ |
...
**Що працює насправді:** у меню магнітоли вимкнути синхронізацію часу по GPS і
виставити час вручну. Тоді зсув GPS вже не зіб'є годинник.

**Що робить застосунок:** читає `567` (з секундами) і веде серію розходжень із
годинником телефона. Час це не виправляє, зате розрізняє три несправності, які з
боку виглядають однаково — «час в авто злітає»:

| Форма розходження | Причина | Що робити |
|---|---|---|
| стрибок (різка зміна за секунди) | підмінений час по GPS або перезавантаження магнітоли | вимкнути синхронізацію часу по GPS |
| рівномірний хід (розходження росте лінійно) | кварц RTC у магнітолі йде не з тією швидкістю | **вимикання GPS не допоможе взагалі**; ремонт/заміна магнітоли |
| скидання після стоянки | магнітола втрачає постійне живлення | запобіжник пам'яті, живлення AVN |

Саме тому потрібні секунди, а не хвилини: на хвилинній роздільності швидкість ходу
міряється годинами, на секундній — за кілька хвилин поїздки.

Поріг «це стрибок, а не хід» — 30 с за хвилину: навіть дуже поганий кварц стільки
не набігає. Стрибок обриває серію, бо швидкість, порахована через переставлений
годинник, — це величина стрибка, а не швидкість ходу.
