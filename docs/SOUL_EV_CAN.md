...
|---|---|---|---|
| `4B0` | швидкості **чотирьох коліс** окремо | по 16 біт LE ÷ 30 → км/год; пари (1,0)(3,2)(5,4)(7,6) | ні — звірка з `4F0` і розбігом у повороті |
| `433` | стоянкове гальмо | байт 2, біт 3 | ні — підняти й опустити |
| `050` | світло (байт 1, маска 0x03), склоочисники (байт 2, маска 0x07 + байт 1, маска 0xF0), покажчики (байт 2, маска 0x30) | біти | ні |
| `567` | годинник авто | байти 1, 2, 3 — години, хвилини, секунди | ні — секунди мусять рахувати одна за одною |
| `4F2` | швидкість (байт 1 ÷ 2) і **запалювання** (байт 2, біти 6-7) | див. нижче: їхню формулу швидкості виправлено | запалювання — так, порівнянням знімків шини |
...
## Що з перенесеного НЕ працює на цій машині

Перша поїздка з новими полями (журнал `journal_22`) дала два вироки, і обидва
зафіксовані в перевірках `PortedValues`, а не лише тут:

| поле | що прийшло | вирок |
|---|---|---|
| знос комірок, `21 05` зміщення 27-32 | `wear=0/№1 best=0/№1` | **не працює**: нульовий знос усіх 96 комірок при живій батареї — це не число, а не те поле |
| межі потужності, `21 01` зміщення 7-10 | `okOut=90 okIn=90` | **не працює**: дві межі різної природи не бувають рівні, та ще й обидві рівно 90 |
| три температури пакета, `21 05` | `tMax=26 tMin=26 tIn=26` | **під підозрою**: разом із вісьмома однаковими модулями схоже на один байт, прочитаний одинадцять разів; вирішиться після швидкої зарядки, коли пакет справді нерівний |
| `21 01` зміщення 31, акумулятор 12 В | `v12=14.1` | **працює**: понад 13.5 В означає, що DC-DC живий |
...
даних, — воно живе на вкладці «Системи» з позначкою й не потрапляє в жоден
розрахунок.

## Чому цю таблицю не можна брати на віру

Той самий SoulEVSpy розбирає кадр `4F2` як «точну швидкість»:

```java
mSpeedKmH = (getDataByte(1) | ((getDataByte(2) & 0x80) << 1)) / 2.0;
```

На цій машині, знятій **стоячи на місці з увімкненим запалюванням**, кадр був
`4F2 20 00 C0 50 00 00 00 B0`. Біт 7 байта 2 стоїть, отже за їхньою формулою
швидкість = `(0 | 0x100) / 2` = **128 км/год** у нерухомого авто.

Бо цей біт тут — не дев'ятий біт швидкості, а **біт запалювання**: його знайшли
порівнянням двох знімків шини, до і після повороту ключа. Разом із ним у `4F2`
зʼявляється біт 6 байта 2 і біти 4, 5, 7 байта 7, а на шині — девʼять кадрів,
яких при вимкненому запалюванні немає взагалі: `164, 200, 201, 202, 291, 2A1,
4F0, 523, 524`.

Тому кожне число з таблиці або вже зійшлося з нашими вимірами, або перевіряється
на живій машині перед тим, як йому вірити.
